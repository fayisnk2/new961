...
from info import temp, CHANNELS, ADMINS, AUTH_CHANNEL, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, GRP_LNK, REQST_CHANNEL, SUPPORT_CHAT_ID, MAX_B_TN, IS_VERIFY, HOW_TO_VERIFY
from utils import get_settings, get_size, is_subscribed, save_group_settings, verify_user, check_token, check_verification, get_token, send_all
...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        try: invite_link = await client.create_chat_invite_link(int(AUTH_CHANNEL))
        except ChatAdminRequired: return logger.error("Mᴀᴋᴇ sᴜʀᴇ Bᴏᴛ ɪs ᴀᴅᴍɪɴ ɪɴ Fᴏʀᴄᴇsᴜʙ ᴄʜᴀɴɴᴇʟ")
        btn = [[InlineKeyboardButton(text="❆ Jᴏɪɴ Oᴜʀ Bᴀᴄᴋ-Uᴘ Cʜᴀɴɴᴇʟ ❆", url=invite_link.invite_link)]]
    ...
    if btn:= await create_request_forcesub_buttons(message.from_user.id):
        if message.command[1] != "subscribe":
            try:
                kk, file_id = message.command[1].split("_", 1)
                pre = 'checksubp' if kk == 'filep' else 'checksub' 
                btn.append([InlineKeyboardButton(text="↻ Tʀʏ Aɢᴀɪɴ", callback_data=f"{pre}#{file_id}")])
            except (IndexError, ValueError):
                btn.append([InlineKeyboardButton(text="↻ Tʀʏ Aɢᴀɪɴ", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")])
        return await client.send_message(
            chat_id=message.from_user.id,
            text="<b>Please request Join the Following Channels to use this Bot!</b>",
            reply_markup=InlineKeyboardMarkup(btn),
            parse_mode=enums.ParseMode.HTML)
...
@Client.on_message(filters.command("purgeone") & filters.user(ADMINS))
...
@Client.on_message(filters.command("purgetwo") & filters.user(ADMINS))
