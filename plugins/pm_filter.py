...
from utils import get_size, is_subscribed, get_poster, get_settings, save_group_settings, get_shortlink, send_all, check_verification, get_token
...
async def give_filter(client, message):
    if message.chat.id != SUPPORT_CHAT_ID:
        if await global_filters(client, message): return
        if await manual_filters(client, message): return
        settings = await get_settings(message.chat.id)
        try:
            if settings['auto_ffilter']: await auto_filter(client, message)
        except KeyError:
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                if clicked == typed: return await query.answer(url=f"https://t.me/{temp.U_NAME}?start={ident}_{file_id}")
                else: await query.answer(f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. Rᴇǫᴜᴇsᴛ Yᴏᴜʀ's !", show_alert=True)
            
            if await create_request_forcesub_buttons(query.from_user.id):
                if clicked == typed: return await query.answer(url=f"https://t.me/{temp.U_NAME}?start={ident}_{file_id}")
                else: await query.answer(f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. Rᴇǫᴜᴇsᴛ Yᴏᴜʀ's !", show_alert=True)
        ...
        if await create_request_forcesub_buttons(query.from_user.id):
            return await query.answer("Rᴇǫᴜᴇsᴛ Jᴏɪɴ ᴛᴏ ᴏᴜʀ Bᴀᴄᴋ-ᴜᴘ ᴄʜᴀɴɴᴇʟ ᴍᴀʜɴ! 😒", show_alert=True)
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            logger.exception(e)
            fek = await message.reply(text=cap, reply_markup=InlineKeyboardMarkup(btn))
            try:
                if settings['auto_delete']:
                    await asyncio.sleep(60)
                    await fek.delete()
