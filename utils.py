...
async def send_all(bot, userid, files, ident):
    if AUTH_CHANNEL and not await is_subscribed(bot=bot, userid=userid):
        try:
            ...
            return logger.error("Mᴀᴋᴇ sᴜʀᴇ Bᴏᴛ ɪs ᴀᴅᴍɪɴ ɪɴ Fᴏʀᴄᴇsᴜʙ ᴄʜᴀɴɴᴇʟ")
            
        if ident == 'filep' or 'checksubp': pre = 'checksubp'
        else: pre = 'checksub'
    ...
    if btn:= await create_request_forcesub_buttons(userid):
        if ident == 'filep' or 'checksubp': pre = 'checksubp'
        else: pre = 'checksub' 
        btn.append([InlineKeyboardButton(text="↻ Tʀʏ Aɢᴀɪɴ", callback_data=f"{pre}#send_all")])
        await bot.send_message(
            chat_id=userid,
